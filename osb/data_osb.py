...
path = 'Z:/COMERCIAL/PEDIDOS DE BOMBAS - OSB/OSB/OSB/OSB-' + year

#path = (r'Z:/COMERCIAL/PEDIDOS DE BOMBAS - OSB/OSB/OSB/OSB-2025')
    
cot_folders = os.listdir(path)

cot_folders = cot_folders[:len(cot_folders)]
...
#        df = df.drop(index = 4)
        ...
        osb_check = df.iloc[:,0].str.contains("ORDEM DE FABRICAÇÃO:|ORDEM DE COMPRA:|PEDIDO")
        
        osb_check = pd.DataFrame(osb_check)
        
        osb_loc = osb_check[osb_check[0]==True].index.values.astype(int)
    ...
    #    set_price = df[df.eq("PREÇO TOTAL, 01 CONJUNTO:|PRECIO TOTAL, 01 BOMBA:").any(axis=1)].dropna(axis = 1, how = 'all', ignore_index = True)
        ...
        for i in range(len(tec_data)):
            ...
            osb_number = df.iloc[osb_loc[i],0].upper().replace("ORDEM DE FABRICAÇÃO: ", "")
...
refxlsx = 'C:/Users/gustavo.andrade/Downloads/osbreg' + year[-2:] + '.xlsx'

refcsv = 'C:/Users/gustavo.andrade/Downloads/osbreg' + year[-2:] + '.csv'
